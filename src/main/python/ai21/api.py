...
def get_api_params(api: str, **kwargs) -> Dict:
    """
    Forms the parameter set for various supported APIs.
...
def sentence_sentiment_labeler(sentence, max_tokens=114, temperature=0.33, model='j1-grande'):
    ...
    resp, _ = api_request(prompt, num_results=1, max_tokens=max_tokens, temperature=temperature,
                          top_returns=0, top_percentile=1.0,
                          stop_sequences=["##"], api='complete', model=model)
