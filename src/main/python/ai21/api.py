...
API_COST = {'j1-large': 0.0003, 'j1-grande': 0.0008, 'j1-jumbo': 0.0128}
...
def get_api_url(model_type: str = 'complete', model: str = 'j1-large', custom_model: Union[None, str] = None) -> str:
    """
    ...
    if model_type == 'summarization':
        ...
        if custom_model is not None:
            model_str += f"/{custom_model}"
        url = f"https://api.ai21.com/studio/v1/{model_str}/complete"
    else:
...
def api_request(prompt: str, num_results: int = 1, max_tokens: int = 16, temperature: float = 1.0,
                ...
                custom_model: Union[None, str] = None,
    ...
    if config is None:
        ...
        response = requests.post(get_api_url(model_type=model_type, model=model, custom_model=custom_model),
...
def get_response_cost(response: requests.models.Response) -> float:
    ...
    """
    num_tokens_generated = len(response.json()['completions'][0]['data']['tokens'])
    model = [s for s in response.request.path_url.split('/') if s.startswith('j1')][0]
